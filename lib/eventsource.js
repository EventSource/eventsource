...
function EventSource (url, eventSourceInitDict) {
  ...
  function scheduleReconnect () {
    ...
    // TODO emit this inside the attempt so we can include the attempt number in the event
    ...
    operation.attempt(function (currentAttempt) {
      // var event = new Event('retrying', { retryAttempt: numAttempt})
      if (readyState !== EventSource.RETRYING) return
      connect()
      if (operation.retry(readyState === EventSource.RETRYING)) {
        return
      }
    })
  }
  ...
  function onConnectionFailed (error) {
    ...
    if (isRetryableError(errorEvent)) {
      ...
      // TODO don't think we want this. It's causing the connection to be closed
      // _emit(errorEvent)
      scheduleReconnect()
    } else {
      _emit(errorEvent)
