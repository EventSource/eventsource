...
function EventSource (url, eventSourceInitDict) {
  ...
  function scheduleReconnect () {
    ...
    // TODO - the first attempt of each strategy is fired off at the same time.
    //  This will need improved so each strategy only activated after the previous is spent. One solution is to put
    //  them all on one horrible nested block, but there should be a more readable way.
    //  The minTimeout property of medium and long term strategies mean they will do subsequent retries correctly which
    //  should be good enough for now

    // The first retry scenario covering the first 3 seconds of retrying.
    // We want to retry three times after the initial try to account for intermittent network issues.
    // Exponential backoff not used here.
    var shortTermRetryStrategy = retry.operation({
      retries: 3,
      factor: 0,
      minTimeout: 1000,
      maxTimeout: 1000,
    ...
    var mediumTermStrategy = retry.operation({
      retries: 15,
      factor: 3,
      minTimeout: 10000,
      maxTimeout: 15000,
  ...
  function onConnectionFailed (error) {
    ...
    // If we're already in the midst of a retry then we don't want to retry again.
    if (readyState === EventSource.RETRYING) {
      return
    }

    var eventType = error ? 'error' : 'end'
    var errorEvent = error ? new Event('error', {error: error}) : new Event('end')
    if (isRetryableError(errorEvent)) {
      readyState = EventSource.RETRYING
      _emit(eventType, errorEvent)
      scheduleReconnect()
    } else {
      _emit(eventType, errorEvent)
  ...
  function connect () {
    ...
    req.on('error', function (err) {
      // We only want to emit the error if we aren't retrying
      if (readyState !== EventSource.RETRYING) {
        // Emit the error so we can handle it
        onConnectionFailed(err.message)
      }

      // If we are retrying, we want to return the error instead of emitting it so that retry library
      // knows we need to retry again.
      return err
    })
    ...
    req.on('timeout', function () {
      onConnectionFailed({
        message: 'Read timeout, received no data in ' + timeout +
